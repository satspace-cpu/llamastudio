...
public partial class ChatViewModel : ObservableObject
{
    ...
    async Task ResolvePortAsync()
    {
        var profile = await GetActiveProfileAsync();
        int port = profile != null && profile.Port > 0 ? profile.Port : (_settings.DefaultPort > 0 ? _settings.DefaultPort : 8080);
        Dispatcher.UIThread.Post(() => Port = port);
    }
