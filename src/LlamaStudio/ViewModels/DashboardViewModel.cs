...
public partial class DashboardViewModel : ObservableObject, IDisposable
{
    ...
    readonly DispatcherTimer _updateCheckTimer;
  ...
  static readonly double s_totalRamGb = GetTotalPhysicalMemory() / (1024.0 * 1024.0 * 1024.0);
    ...
    async Task SwitchProfileAsync()
    {
        ...
        // Apply host/port from settings (only if profile doesn't have its own)
        if (string.IsNullOrWhiteSpace(SelectedProfile.Host) && !string.IsNullOrWhiteSpace(_settings.DefaultHost))
            SelectedProfile.Host = _settings.DefaultHost;
        if (SelectedProfile.Port <= 0 && _settings.DefaultPort > 0)
        ...
        MonitoringViewModel? monitoringVm = null)
    {
        ...
        // Periodic update check every 5 minutes (using static JSON, no API rate limits)
        _updateCheckTimer = new DispatcherTimer { Interval = TimeSpan.FromMinutes(5) };
        _updateCheckTimer.Tick += (_, _) => _ = CheckAppUpdatesAsync();
    ...
    async Task CheckAppUpdatesAsync()
    {
        try
        {
            ...
            await Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(() =>
            {
                ...
                {
                    HasAppUpdate = false;
                    _log.Information($"App is up to date: {AppVersion}", "Dashboard");
                }
            });
        }
        catch (Exception ex)
        {
            // Don't reset HasAppUpdate on error - keep previous state
            _log.Warning($"Failed to check app updates (keeping previous state): {ex.Message}", "Dashboard");
        }
    }
    ...
    async Task UpdateAppAsync()
    {
        try
        {
            ...
            var restartNow = await _dialog.ShowConfirmationAsync(
                $"Обновление {AppLatestVersion} скачано.\n\nПерезапустить приложение сейчас для применения обновлений?",
                "Обновление готово");

            if (restartNow)
            {
                await ApplyUpdateAndRestart(downloadedPath);
            }
            else
            {
                // Save path for later restart
                File.WriteAllText(Path.Combine(tempDir, "pending_update.txt"), downloadedPath);
                await _dialog.ShowMessageAsync(
                    $"Обновление {AppLatestVersion} готово к установке.\n\nПриложение будет перезапущено при следующем запуске.",
                    "Обновление готово");
            }
        }
        catch (Exception ex)
    ...
    async Task ApplyUpdateAndRestart(string downloadedPath)
    {
        var currentExe = System.Diagnostics.Process.GetCurrentProcess().MainModule?.FileName;
        if (!string.IsNullOrEmpty(currentExe))
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "LlamaStudio_Update");
            var logFile = Path.Combine(tempDir, "update_log.txt");
            var restartScript = Path.Combine(tempDir, "restart.bat");
            
            var escapedCurrent = currentExe.Replace("%", "%%");
            var escapedNew = downloadedPath.Replace("%", "%%");
            var escapedLog = logFile.Replace("%", "%%");
            var escapedScript = restartScript.Replace("%", "%%");

            File.WriteAllText(restartScript, $@"@echo off
setlocal
echo [{{date}} {{time}}] Starting update process > ""{escapedLog}""
echo Current EXE: {escapedCurrent} >> ""{escapedLog}""
echo Downloaded EXE: {escapedNew} >> ""{escapedLog}""

:: Kill any remaining processes
taskkill /F /IM LlamaStudio.exe >> ""{escapedLog}"" 2>&1
timeout /t 3 /nobreak >nul

:: Replace the executable
echo Deleting old EXE... >> ""{escapedLog}""
del /F /Q ""{escapedCurrent}"" >> ""{escapedLog}"" 2>&1
timeout /t 1 /nobreak >nul

echo Copying new EXE... >> ""{escapedLog}""
copy /Y ""{escapedNew}"" ""{escapedCurrent}"" >> ""{escapedLog}"" 2>&1

if exist ""{escapedCurrent}"" (
    echo Update successful! Starting new version... >> ""{escapedLog}""
    timeout /t 1 /nobreak >nul
    start """" ""{escapedCurrent}""
    :: Clean up after successful start
    ping 127.0.0.1 -n 2 >nul
    del ""{escapedScript}"" >> ""{escapedLog}"" 2>&1
) else (
    echo ERROR: New file not found after copy! >> ""{escapedLog}""
    pause
)
");

            _log.Information($"Update script created: {restartScript}", "Dashboard");
            _log.Information($"Current EXE: {currentExe}", "Dashboard");
            _log.Information($"Downloaded EXE: {downloadedPath}", "Dashboard");
